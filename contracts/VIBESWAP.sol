...
/**
 * @title VIBESWAP
 * @notice Automated Market Maker (AMM) DEX contract for token swaps on VIBESWAP
 ...
 * @dev Fee model: 0.3% swap fee distributed to liquidity providers (Uniswap V2 style)
...
contract VIBESWAP {
     ...
     * @dev SECURITY: Follows CEI pattern - all state updates occur BEFORE external calls
     * @dev Fee model: 0.3% fee is applied to input amount and distributed to liquidity providers
     *      The fee is retained in the pool reserves (Uniswap V2 style)
     */
    function swap(
        address tokenIn,
        address tokenOut,
        ...
        returns (uint256 amountOut)
    {
        ...
        // Calculate output amount (fee is applied internally in _calculateSwapOutput)
        ...
        // Calculate actual fee amount for event emission
        // Fee = 0.3% of input amount, retained in pool reserves
        uint256 feeAmount = (amountIn * SWAP_FEE) / FEE_DENOMINATOR;

        // EFFECTS: Update state variables BEFORE external calls (CEI pattern)
        // Input reserve increases by full amountIn (includes fee)
        // Output reserve decreases by amountOut
        if (tokenIn == token0) {
     ...
     * @dev Applies 0.3% fee to input amount before calculating output
    ...
    function _calculateSwapOutput(
        ...
        // Apply 0.3% fee: amountInWithFee = amountIn * (1 - 0.003) = amountIn * 0.997
